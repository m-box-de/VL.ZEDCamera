	...
	public class ZEDCamera
	{
		...
		*/
		...
		private static void DebugMethod(string message)
		{
			...
			Console.WriteLine("[ZED plugin]: " + message);
		}
		...
		*/
		...
		public static bool CheckPlugin()
		{
			...
			if (env != null) //Found path to the ZED SDK in PC's environmental variables. 
			{
				...
				if (error) {
					Console.WriteLine(ZEDLogMessage.Error2Str(ZEDLogMessage.ERROR.SDK_NOT_INSTALLED)); //Print that SDK isn't installed. 
					return false;
					...
					try {
						if (dllz_check_plugin(PluginVersion.Major, PluginVersion.Minor) != 0) { //0 = installed SDK is compatible with plugin. 1 otherwise.
							Console.WriteLine(ZEDLogMessage.Error2Str(ZEDLogMessage.ERROR.SDK_DEPENDENCIES_ISSUE));
							return false;
						}
					}
			...
			{
				Console.WriteLine(ZEDLogMessage.Error2Str(ZEDLogMessage.ERROR.SDK_NOT_INSTALLED));
				return false;
			}
		...
		static private bool CheckDependencies(string[] filesFound)
		{
			...
			foreach (string dependency in dependenciesNeeded)
			{
				...
				{
					isASDKPb = true;
					Console.WriteLine("[ZED Plugin ] : " + dependency + " is not found");
				}
			}
