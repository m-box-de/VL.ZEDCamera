...
namespace sl
{
    ...
    public class ZEDCamera
    {
        ...
        /// </summary>
        private Dictionary<int, Dictionary<int, Texture>> textures;
        ...
        */
        ...
        public static Quaternion Matrix4ToQuaternion(Matrix m)
        {
        ...
        {
            //Create the textures
            textures = new Dictionary<int, Dictionary<int, Texture>>();
        ...
        public ERROR_CODE Init(ref InitParameters initParameters)
        {
            ...
            if (imageWidth > 0 && imageHeight > 0)
            {
                ...
                fov_H = MathUtil.DegreesToRadians(calibrationParametersRectified.leftCam.hFOV) ;
                fov_V = MathUtil.DegreesToRadians(calibrationParametersRectified.leftCam.vFOV);
        ...
        public void FillProjectionMatrix(float zFar = 500, float zNear = 0.1f)
        {
            ...
            float fovx = MathUtil.DegreesToRadians(parameters.leftCam.hFOV);
            float fovy = MathUtil.DegreesToRadians(parameters.leftCam.vFOV);
            ...
            projection[0, 0] = (float)(1.0f / Math.Tan(fovx * 0.5f)); //Horizontal FoV.
            projection[0, 1] = 0;
            projection[0, 2] = 2.0f * ((f_imageWidth - 1.0f * parameters.leftCam.cx) / f_imageWidth) - 1.0f; //Horizontal offset.
            projection[0, 3] = 0;

            projection[1, 0] = 0;
            projection[1, 1] = (float)(1.0f / Math.Tan(fovy * 0.5f)); //Vertical FoV.
        ...
        /// </summary>
        private void RegisterTexture(Texture m_Texture, int type, int mode)
        {
        ...
        public Texture CreateTextureImageType(GraphicsDevice device, VIEW mode, Resolution resolution = new Resolution())
        {
            ...
            {
                textures.Add((int)TYPE_VIEW.RETRIEVE_IMAGE, new Dictionary<int, Texture>());
            }
        ...
        /// <param name="resolution">Resolution of the image. Should correspond to ZED's current resolution.</param>
        /// <returns>Texture that will update each frame with the ZED SDK's output.</returns>
        /*public Texture CreateTextureMeasureType(MEASURE mode, Resolution resolution = new Resolution())
        {
            ...
            {
                m_Texture = new Texture(width, height, TextureFormat.RGBAFloat, false, true);
            }
            ...
            {
                m_Texture = new Texture(width, height, TextureFormat.RFloat, false, true);
            }
            else
            {
                m_Texture = new Texture(width, height, TextureFormat.RGBA32, false, true);
            }
            if (!((uint)resolution.width == 0 && (uint)resolution.height == 0))
            {
            ...
            {
                textures.Add((int)TYPE_VIEW.RETRIEVE_MEASURE, new Dictionary<int, Texture>());
            }
        ...
        public Texture GetTexture(TYPE_VIEW type, int mode)
        {
        ...
        public float GetFOV()
        {
            return MathUtil.DegreesToRadians( GetCalibrationParameters(false).leftCam.vFOV );
        }
        ...
        static public void Float2Matrix(ref Matrix m, float[] f)
        {
            if (f == null) return;
